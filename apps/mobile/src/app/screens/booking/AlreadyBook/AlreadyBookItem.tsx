...
import {
  fetchCurrentRoomBookingDetail
} from "../../../redux/features/room-booking/thunk/fetch-current-booking-detail.thunk";
...
const AlreadyBookItem: React.FC<AlreadyBookItemProps> = (props) => {
  ...
  const handleViewAlreadyBookingDetail = (id: string) => {
    dispatch(fetchCurrentRoomBookingDetail(id)).unwrap()
      .then((e) => {
        console.log(e);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>
          {props.item.status === 'BOOKING' ? 'Room is booking' : `Room ${props.item.roomName}`}
        </Text>

        <View style={[styles.chipContainer, getRoomBookingColorChip()]}>
          <Text style={[styles.chipTextContent]}>
            {props.item.status}
          </Text>
        </View>
      </View>
      <View style={styles.bodyContainer}>
        ...
        <View style={styles.roomBookingInfo}>
          ...
          {props.item.status === 'BOOKING' ? <>
            ...
            <Text style={{ color: BLACK, fontWeight: '500' }}>Requested at: {dayjs(props.item.requestedAt).format('HH:mm:ss DD/MM/YYYY')}</Text>

          </> : null}
          {props.item.status !== 'BOOKING' ? <Text style={{ color: BLACK, fontWeight: '500' }}>Booked at: {dayjs(props.item.bookedAt).format('HH:mm:ss DD/MM/YYYY')}</Text> : null}
          {props.item.status === 'BOOKED' ? <>
            <Text style={{ color: BLACK, fontWeight: '500' }}>Checkin at: {dayjs(props.item.timeCheckIn).format('HH:mm:ss DD/MM/YYYY')}</Text>
            <Text style={{ color: BLACK, fontWeight: '500' }}>Checkout at: {dayjs(props.item.timeCheckOut).format('HH:mm:ss DD/MM/YYYY')}</Text>
      ...
      <View style={{
        ...
        <TouchableOpacity
          onPress={() => handleViewAlreadyBookingDetail(props.item.id)} style={{
          height: 30,
