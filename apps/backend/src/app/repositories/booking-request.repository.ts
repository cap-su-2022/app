...
export class BookingRoomRepository extends Repository<BookingRequest> {
  ...
  async isConflictWithStartEndDateTime(date: string, timeStart: string, timeEnd: string): Promise<boolean> {
    /*
SELECT * FROM "booking_request" "booking_request"
    ...
    * */
    return await this.createQueryBuilder('booking_request')
      .select('COUNT(1)', 'count')
      .where('booking_request.status IN (:...status)', {status: ['BOOKED', 'CHECKED_IN']})
      .andWhere(new Brackets((qb) =>
        qb.where('booking_request.checkin_time >= :checkInStartTime', {checkInStartTime: timeStart})
          .orWhere('booking_request.checkout_time <= :checkInEndTime', {checkInEndTime: timeStart})
      ))
      .andWhere('booking_request.checkout_time <= :checkOutTime', {checkOutTime: timeEnd})
